...
class AgentConfig:
    ...
    progress_callback: Callable[[int,int],None]|None = None  # 进度回调
...
class TokenCounter:
    ...
    def add(self, input_tokens: int, cached_tokens: int, output_tokens: int, reasoning_tokens: int, api_total_tokens: int = 0):
        with self.lock:
...
class Agent:
    ...
    def _prepare_request_data(self, prompt: str, system_prompt: str, temperature=None, top_p=None, json_format=False):
    ...
    async def _continue_fetch_async(
        ...
        try:
            ...
            input_tokens, cached_tokens, output_tokens, reasoning_tokens, api_total_tokens = extract_token_info(response_data)
                ...
                if retry_count == 0:
                    if self.total_error_counter.add():
                        ...
                        return (
                            ...
                            else (prompt if error_result_handler is None else error_result_handler(prompt, self.logger))
                        )
    ...
    async def send_prompts_async(
        ...
        if task_id:
            ...
            checkpoint_file = os.path.join(out_dir, f"checkpoint_{task_id}.jsonl")
            ...
            self.logger.warning("未获取到 task_id，本次任务将不使用断点续传缓存功能。")
        # ==============================================================================

        # 2. 精准计算预计发送数
        actual_send_count = sum(1 for p in prompts if p not in cached_results)
        self.logger.info(f"进度统计 - 任务总数: {total} | 命中缓存跳过: {total - actual_send_count} | 预计需发送新请求: {actual_send_count}")
        ...
        async with httpx.AsyncClient(trust_env=False, mounts=proxies, verify=False, limits=limits) as client:
            async def send_with_semaphore(index: int, p_text: str):
                ...
                async with semaphore:
                    ...
                    # 仅在获取到了 checkpoint_file 路径时才写入文件
                    if checkpoint_file:
                ...
                if self.unresolved_error_count == 0:
                    if os.path.exists(checkpoint_file):
                        try:
                            ...
                            self.logger.info("🎉 任务已 100% 成功完成，已自动清理本地专属断点文件。")
                        except OSError as e:
    ...
    def send(
        ...
        try:
            ...
            if finish_reason == "stop":
                ...
                self.logger.warning(f"响应因长度限制被截断，尝试继续获取...")
                return self._continue_fetch(
                    client=client, prompt=prompt, system_prompt=system_prompt,
                    force_json=force_json, pre_send_handler=pre_send_handler,
                    result_handler=result_handler, error_result_handler=error_result_handler,
                    retry_count=retry_count, accumulated_result=result,
                )
            elif finish_reason in ("tool_calls", "function_call"):
                self.logger.warning(f"finish_reason 为 '{finish_reason}'，当前不支持工具调用，返回已获取内容")
                result = self._sanitize_result(result)
                return result if result else (prompt if error_result_handler is None else error_result_handler(prompt, self.logger))
            elif finish_reason == "content_filter":
                self.logger.error(f"响应内容被过滤")
                raise ValueError("内容被过滤")
            elif finish_reason is None:
            ...
            if is_hard_error:
                if retry_count == 0:
                    if self.total_error_counter.add():
                        ...
                        with self.unresolved_error_lock:
                            self.unresolved_error_count += 1
                        return best_partial_result if best_partial_result else (prompt if error_result_handler is None else error_result_handler(prompt, self.logger))
                elif self.total_error_counter.reach_limit():
                    self.logger.error("错误次数过多，已达到上限，不再为该请求重试。")
                    with self.unresolved_error_lock:
                        self.unresolved_error_count += 1
    ...
    def _send_prompt_count(
        ...
        result = self.send(
            client, prompt, system_prompt, force_json=force_json, pre_send_handler=pre_send_handler,
            result_handler=result_handler, error_result_handler=error_result_handler,
        )
        count.add()
        return result
    ...
    def send_prompts(
        ...
        self.logger.info(f"预计发送{len(prompts)}个请求")
        self.total_error_counter.max_errors_count = len(prompts) // MAX_REQUESTS_PER_ERROR
        ...
        with httpx.Client(trust_env=False, mounts=proxies, verify=False, limits=limits) as client:
            clients = itertools.repeat(client, len(prompts))
            with ThreadPoolExecutor(max_workers=self.max_concurrent) as executor:
                results_iterator = executor.map(
                    self._send_prompt_count, clients, prompts, system_prompts, json_formats,
                    counters, pre_send_handlers, result_handlers, error_result_handlers,
                )
